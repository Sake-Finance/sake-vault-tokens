...
describe("Calls", function () {
  ...
  describe("misc", function () {
    // plain calls to test contract
    it("calls test callee", async function () {
      let tx1 = await test1Callee.testFunc1()
      await expect(tx1).to.emit(test1Callee, "Test1Event").withArgs(1)
      let tx2 = await test1Callee.testFunc2()
      await expect(tx2).to.emit(test1Callee, "Test1Event").withArgs(2)
      let tx3 = await test1Callee.testFunc3()
      await expect(tx3).to.emit(test1Callee, "Test1Event").withArgs(3)
    })
  });
